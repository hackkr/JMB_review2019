---
title: "Scaling exploration"
output: html_notebook
---

This notebook compares scaling the expression data different ways. Promted because I noticed that the expression ranges for s. cer data were larger than hbt and curious if there was a better solution than adjusting axes margins in the main figures.. mostly because I'm fairly sure its an artifact of how each datsa set was normalized, but I don't want to investigate that for the purpose of the review.

Which brings me to three (easy) options:
  1. scale() by gene expression... interpretation: single gene expression across all conditions is 0 centered and standardized. Equavalent to scaling across conditions  
...
Elected to use scale across genes rather than within conditions becasue it manitains the noise/spikiness, but evens the amplitude of gene expression between the two species. 
...
y.clust <- read_csv("../output/clustered_genelists/2019-02-08yeast2clusters.csv")
h.clust <- read_csv("../output/clustered_genelists/2019-02-08hbt2clusters.csv")
...
ggplot(aes(x = cond, y = avg_expression, group = cluster, color = cluster)) +
  geom_line(show.legend = F) +
  geom_ribbon(aes(ymin = avg_expression - CI95,ymax = avg_expression + CI95,fill = cluster), color = NA, alpha = 0.3) +
  scale_color_viridis(option = "C", begin = 0.1, end = 0.9) + 
  scale_fill_viridis(option = "C", begin = 0.1, end = 0.9) +
  ylim(-3,3) + xlab('condition') + ylab('Average expression ratio') + theme_classic() +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
